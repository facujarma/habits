...
import { IconCheck, IconMenu4, IconStar, IconStarFilled } from '@tabler/icons-react'
                ...
                <footer className='flex justify-between w-full items-center'>
                        ...
                        {stars.map((star, index) => (
                            <li key={index}>
                                <button
                                    ...
                                    onClick={async () => {
                                        await changeStars(bookID, index + 1)
                                    }}
                                >
                                    {star === "filled" ? (
                                        <IconStarFilled className='text-[#999A66]' />
                                    ) : (
                    ...
                    <Dropdown>
                        ...
                        <DropdownMenu aria-label="Dropdown Variants" onAction={(key) => handleOptionSelected(key)}>
                            <DropdownItem key="edit">{t('edit_book')}</DropdownItem>
                            {finished ? (
                                <DropdownItem key="unfinished">{t('mark_as_not_finished')}</DropdownItem>
                            ) : (
                                <DropdownItem key="finished">{t('mark_as_finished')}</DropdownItem>
                            )}
                            <DropdownItem key="reflexion">{t('add_reflection')}</DropdownItem>
