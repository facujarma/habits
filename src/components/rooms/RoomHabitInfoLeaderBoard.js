                ...
                <ul className="flex flex-col w-full aspect-square">
                    {leaderBoard.map((user, index) => {
                        ...
                        return (
                            <li
                                key={user.name}
                                className={`w-full flex items-center justify-between px-4 py-2 border-b border-[#616161] ${isMe ? 'bg-[#626262]' : ''
                                    }`}
                            >
                                <div className="flex gap-2 items-center h-full">
                                    <span className="text-[#C5C5C5]">
                                        {isFirst ? (
                                            <IconMedal size={18} color="#C7B63B" />
                                        ) : (
                                ...
                                <div className="flex gap-2 items-center h-full">
                                    <span className="border-r-2 border-[#616161] px-2">{user.points}</span>
                                    <span className="flex gap-1 items-center text-[#C5C5C5]">
                                        {user.streak} <IconFlame size={18} color="#C7B63B" />
                                    </span>
                                </div>
                            </li>
                        )
                    ...
                    <li className="opacity-50 w-full flex items-center justify-between px-4 py-2 border-b border-[#616161]">
                        <span className="w-full text-center">{t('no_more_members')}</span>
                    </li>
                </ul>
            )}
        </div>
